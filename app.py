...
def render_demand_forecast_vs_actual(sku_id: str):
    """
    Renders business-facing Demand Forecast vs Actual visualization for a selected SKU.
    Demonstrates:
      1. Historical actual demand experienced by the business.
      2. Predictive demand forecast generated by Sentinel.
      3. Side-by-side alignment on matching dates with clear error metrics.
    """
    st.markdown("---")
    st.subheader(f"📈 Demand Forecast vs Actual — {sku_id}")
    st.caption("What the business actually experienced vs. what Sentinel predicted.")
    
    if not sku_id:
        st.info("Please select a valid Product (SKU) to display demand forecast vs actual comparison.")
        return

    try:
        model, demand_df = agent_tools._get_demand_model()
    except Exception:
        st.info("Forecast vs Actual comparison is unavailable for this dataset.")
        return
    ...
    c1.metric("Forecast Horizon", f"{res['horizon_periods']} Days")
    c2.metric("Actual Average Demand", f"{res['actual_avg']:.1f} u/d")
    c3.metric("Forecast Average Demand", f"{res['forecast_avg']:.1f} u/d")
    if res.get("mae") is not None:
        c4.metric("Forecast Error (MAE)", f"{res['mae']:.1f} u")
    if res.get("mape") is not None:
        c5.metric("Accuracy (MAPE)", f"{res['mape'] * 100:.1f}%")

    st.markdown("<br>", unsafe_allow_html=True)

    # 2. Interactive Line Chart
    aligned_df = res["aligned_df"]
    chart_df = aligned_df.set_index("Date")[["Actual Demand", "Forecast Demand"]]
    
    tab_comp, tab_hist = st.tabs(["📊 Aligned Comparison (Matching Dates)", "📅 Full Demand History & Forecast Overlay"])
    
    with tab_comp:
        st.line_chart(chart_df, use_container_width=True)
        st.caption("Daily demand comparison on matching dates in the validation window.")
        
    with tab_hist:
        full_df = res.get("full_history_df")
        if full_df is not None and not full_df.empty:
            full_chart_df = full_df.set_index("Date")[["Actual Demand", "Forecast Demand"]]
            st.line_chart(full_chart_df, use_container_width=True)
            st.caption("Historical demand trend leading directly into the forecast comparison horizon.")
        else:
            st.line_chart(chart_df, use_container_width=True)

    # 3. Tabular Observation Details
    with st.expander(f"📋 View Daily Observation Data & Variance ({res['horizon_periods']} observations)", expanded=False):
        st.dataframe(aligned_df, use_container_width=True, hide_index=True)
        ...
        with r1:
            if not inv_df.empty and "current_stock" in inv_df.columns and "reorder_point" in inv_df.columns:
                ...
                if not low_stock.empty:
                    ...
                    with st.expander(f"📈 View Forecast vs Actual for {top_sku}", expanded=False):
                        render_demand_forecast_vs_actual(top_sku)
                else:
                    st.markdown("""
                    <div class='decision-card'>
...
def render_create_decision():
    ...
    with st.container(border=True):
        ...
        try:
            inv_df = agent_tools._get_inventory_df()
            sku_list = inv_df["sku_id"].dropna().unique().tolist() if not inv_df.empty and "sku_id" in inv_df.columns else []
            if not sku_list:
                _, dem_df = agent_tools._get_demand_model()
                if dem_df is not None and not dem_df.empty and "sku_id" in dem_df.columns:
                    sku_list = dem_df["sku_id"].dropna().unique().tolist()
        except:
            sku_list = []
        ...
        if target_id:
            ...
            # Demand Forecast vs Actual for selected SKU in Inventory/Demand mode
            if target_type == "Inventory / Demand Issue" and target_id:
                render_demand_forecast_vs_actual(target_id)
...
def render_simulation():
    ...
    if sim_mode == "📦 Inventory Risk & Stockout":
        ...
        with col_results:
            ...
            # Demand Forecast vs Actual comparison for selected simulation SKU
            render_demand_forecast_vs_actual(selected_sku)
